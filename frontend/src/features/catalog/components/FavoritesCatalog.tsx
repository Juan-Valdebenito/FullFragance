...
export function FavoritesCatalog({ className }: { className?: string }) { const { user } = useSession(); const [items,setItems] = useState<Comparison[]>([]); const [loading,setLoading] = useState(true); const [error,setError] = useState("");
  useEffect(() => { api.comparisons(user.city ?? DEFAULT_CITY).then(data => setItems(data.filter(item => [item.product.id, ...(item.product.aliases || [])].some(id => user.favorites.includes(id))))).catch(reason => setError(reason instanceof ApiError ? reason.message : "No se pudieron cargar tus favoritos.")).finally(() => setLoading(false)); }, [user.city, user.favorites]);
