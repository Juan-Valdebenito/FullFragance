...
export function ProductDetail({ productId }: { productId: string }) {
  ...
  return <main className={`container ${styles.main}`}>
    ...
    <section className={styles.product}><div className={styles.visual}>{product.imageUrl ? <Image src={product.imageUrl} alt={`${product.name} de ${product.brand}`} fill priority sizes="(max-width: 800px) 100vw, 50vw" /> : <div className={styles.visualPlaceholder}><span>FF</span></div>}</div><div className={styles.info}><p className="eyebrow">{product.brand}</p><h1>{product.name}</h1><p className={styles.unit}>{product.unit} · {product.gender}</p><p className={styles.description}>{product.source === "falabella-cl" ? "Producto sincronizado desde Falabella. Si el detalle fue bloqueado, el precio se completará cuando la fuente vuelva a responder." : "Una fragancia seleccionada para comparar precios disponibles."}</p><div className={styles.tags}><span>{product.category}</span>{product.notes.map(note => <span key={note}>{note}</span>)}</div><div className={styles.best}><div><small>Mejor precio disponible</small><strong>{sortedPrices[0] ? money.format(sortedPrices[0].price) : "Precio pendiente"}</strong><span>{sortedPrices[0]?.storeName ?? (product.source === "falabella-cl" ? "Falabella" : "Sin tienda")}</span></div><FavoriteButton productId={product.id} large /></div></div></section>
