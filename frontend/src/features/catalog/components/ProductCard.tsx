...
export function ProductCard({ product, recommendation = false }: { product: Product; recommendation?: boolean }) {
  return <article className={`${styles.card} ${recommendation ? styles.recommendation : ""}`}>
    <div className={styles.imageWrap}><Link href={`/perfumes/${product.id}`} aria-label={`Ver ${product.name}`}>{product.image ? <Image src={product.image} alt={`Perfume ${product.name} de ${product.brand}`} fill sizes="(max-width: 700px) 100vw, 400px" /> : <span className={styles.imagePlaceholder}>FF</span>}</Link>{product.badge && <span className={styles.badge}>{product.badge}</span>}<FavoriteButton productId={product.id}/></div>
